...
for patient_index in range(num_patients):
    ...
    '''dialysate volume'''
    # Generate the original V array
    V = np.zeros(241)
    V[240] = 149  # ml
    V_fill = 233   # ml
    Vr = 40       # ml
    V[0] = V_fill + Vr 
    df_V = np.array([V[0], V[240]])
    f_V = interp1d([0, 240], df_V)
    interpolated_V = f_V(range(0, 241))
    V = np.transpose(interpolated_V)

    # Define the interpolation function for the original data
    f_V_original = CubicSpline([0, 240], df_V)

    # Define the additional time points starting from t=241
    additional_time_points = np.arange(241, 481)

    # Calculate the incremental volume change from V[240] using the original interpolation function
    incremental_volume_change = f_V_original(additional_time_points) - V[240]

    # Generate the extended V array by adding the incremental volume change to V[240]
    V_extended = np.concatenate(([V[240]], V[240] + incremental_volume_change))


    V_minus = V[:120]
    V_plus = list(V) + list(V_extended[1:])
    ...
    for t in t_values:    
        for j, mode in enumerate(modes):
            ...
            SCR = ((patient_data['Plasma_Solute_Concentration']['Creatinine'])/10)*molecular_weight['Creatinine'] #serumc creatinine concentration mg/dL
...
# Calculate the mean for each solute and each protein intake level --> this doesnt take into account time
mean_clearances = np.mean(reshaped_clearances_array, axis=1)

#time values=[120,240,480]
#solute-DPI-time concentration
mean_values_DPI_time={
...
mean_values_DPI_time_array=np.array(list(mean_values_DPI_time.values()))    
reshaped_mean_values_DPI_time=mean_values_DPI_time_array.reshape(4,3,3)
mean_UR_clearances=reshaped_mean_values_DPI_time[0,:,:]
mean_CR_clearances=reshaped_mean_values_DPI_time[1,:,:]
mean_PhR_clearances=reshaped_mean_values_DPI_time[2,:,:]
mean_PoR_clearances=reshaped_mean_values_DPI_time[3,:,:]
...
std_values_DPI_time_array=np.array(list(std_values_DPI_time.values()))    
reshaped_std_values_DPI_time=std_values_DPI_time_array.reshape(4,3,3)
std_UR_clearances=reshaped_std_values_DPI_time[0,:,:]
std_CR_clearances=reshaped_std_values_DPI_time[1,:,:]
std_PhR_clearances=reshaped_std_values_DPI_time[2,:,:]
std_PoR_clearances=reshaped_std_values_DPI_time[3,:,:]
...
# Define the DPI and time values
DPI_values = ['low', 'mid', 'high']
t_values = [120,240,480]

fig, ax = plt.subplots(4, 1, figsize=(10,40), sharex=True ) #sharey=True #if I want comparable axis for each solute 
...
# Nested loop to iterate over solutes and DPI-time combinations
...
xtick_labels = [f" Mean DPI= {mean_DPI[DPI_key]}, time={t}" for DPI_key in DPI_values for t in t_values]
for axis in ax:
    axis.set_xticks(range(len(DPI_values) * len(t_values)))
    axis.set_xticklabels(xtick_labels, rotation=35, fontsize=8)
    
fig.supxlabel('Mean DPI (g/day), time (min)')
fig.supylabel('Total solute removal (mmol)')
fig.suptitle('Influence of Dwelltime on Solute Clearance in relation to Daily Protein Intake (DPI) in Females', fontsize=14)

plt.tight_layout()
plt.show()


fig, ax = plt.subplots(1,1, figsize=(10,20)) # If you want a single subplot
# fig, ax = plt.subplots(1, 1, figsize=(10, 20), sharey=True) # If you want comparable y-axis for each solute
...
for i, (solute, mean_values, std_values) in enumerate(zip(solute_name, mean_clearance, std_clearance)):
    for j, DPI_value in enumerate(DPI_values):
        for k, gluc_value in enumerate(t_values):
            # Compute the index for xtick
            xtick_index = j * len(t_values) + k
...
xtick_labels = [f" Mean DPI= {mean_DPI[DPI_key]}, time={t}" for DPI_key in DPI_values for t in t_values]
ax.set_xticks(range(len(DPI_values) * len(t_values)))
ax.set_xticklabels(xtick_labels, rotation=45)

fig.supxlabel('Mean DPI (g/day), time (min)')
fig.supylabel('Total solute removal (mmol)')
fig.suptitle('Influence of Dwelltime on Urea Clearance in relation to Daily Protein Intake (DPI) in Females', fontsize=14)
...
#→ elimination in mg dividing by the total time, 240 min
...
Elimination_rate= {
    'UR':mean_clearance_mg['UR']/240,
    'CR':mean_clearance_mg['CR']/240,
    'PhR':mean_clearance_mg['PhR']/240,
    'PoR':mean_clearance_mg['PoR']/240
    }
...
file_path = 'trail1_female_patients_time.json'
...
df_from_json = pd.read_json('trail1_female_patients_time.json')

# Save DataFrame to a CSV file
df_from_json.to_csv('trial1_female_patients_time.csv', index=False)
