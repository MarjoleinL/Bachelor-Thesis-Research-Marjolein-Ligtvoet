...
V[480] = 149 #ml
V_fill =  233 #ml
Vr_values = np.array([20, 40, 60]) #ml values for -50%, baselineand +50%
for Vr in Vr_values:
    V[0] = V_fill + Vr   
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for Vr20
...
for i in range(solute_count):  
    ...
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus50[i], height=bar_width, label=('Vr 60'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for Vr 20 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus50[i], height=bar_width, label=('Vr 20'),
...
plt.suptitle('Change in Solute Clearance Due to 50% Residual Volume Variation', fontsize=16)
