...
patient_data = {}  # Dictionary (smaller, patient specific) to store data for each patient, so cp, cd, v, mtac (she said clearances ad predicted cp); is replenished inside the loop 
...
#Volumes for -20%, baseline and +20%
VDR_values = np.array([8000, 10000, 12000])


for VDR in VDR_values:
    for j, mode in enumerate(modes):
        #["Urea", "Creatinine", "Sodium", "Phosphate", "Glucose", "Potassium"])
...
# Calculate percentage change compared to VDR 10000
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for VDR8000
...
for i in range(solute_count):  
    # Plot bars for VDR 12000
    print(sensitivity_change_plus20[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus20[i], height=bar_width, label=('VDR 12000'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for VDR 8000 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus20[i], height=bar_width, label=('VDR 8000'),
    ...
    for j, val in enumerate(sensitivity_change_minus20[i]):
        if val<=-10:
            ...
            axes[i].annotate(f'{val:.6f}%', xy=(val, j + bar_width/2), xytext=(-52, 0), textcoords='offset points',
                             fontsize=8, color='black', ha='left', va='center')    
    ...
    axes[i].legend(fontsize='small', loc='lower left') 
    axes[i].set_yticks(np.arange(4)) 
    axes[i].set_yticklabels(modes)
    
plt.suptitle('Change in Solute Clearance Due to 20% VDR Variation', fontsize=16)
