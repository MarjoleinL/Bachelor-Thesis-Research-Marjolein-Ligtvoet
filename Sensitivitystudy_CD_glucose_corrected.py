...
#Values for different available glucose concentrations
...
# Calculate percentage change compared to cd_glucose 163
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for cd_gluc 75.5
...
for i in range(solute_count):  
    # Plot bars for cd_glucose 214
    print(sensitivity_change_plus20[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus20[i], height=bar_width, label=('cd_glucose 214'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for cd_glucose 75.5 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus20[i], height=bar_width, label=('cd_glucose 75.5'),
...
plt.suptitle('Change in Solute Clearance Due to Glucose Dialysate Concentration (CD) Variation', fontsize=16)
