...
#t = 480 #min
#time values for -25%, baseline and +25%
t_values = np.array([360, 480, 600]) 
...
interpolated_V = f_V(range(0, 481))
V = np.transpose(interpolated_V)

# Define the interpolation function for the original data
f_V_original = CubicSpline([0, 480], df_V)

# Define the additional time points starting from t=481
additional_time_points = np.arange(481, 601)

# Calculate the incremental volume change from V[480] using the original interpolation function
incremental_volume_change = f_V_original(additional_time_points) - V[480]

# Generate the extended V array by adding the incremental volume change to V[480]
V_extended = np.concatenate(([V[480]], V[480] + incremental_volume_change))
...
sensitivity_results= sensitivity_array.reshape(4, -1, 4) 

# Calculate percentage change compared to t=480
sensitivity_change_minus20 = ((sensitivity_results[:, 0, :] - sensitivity_results[:, 1, :]) / sensitivity_results[:, 1, :]) * 100
sensitivity_change_plus20 = ((sensitivity_results[:, 2, :] - sensitivity_results[:, 1, :]) / sensitivity_results[:, 1, :]) * 100
...
solute_colors = ['DarkGreen','Navy','DarkOrange','DarkViolet']  # Define colors for each solute for sensitivity_change_plus20
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for sensitivity_change_minus20
...
plt.subplots_adjust(top=1, hspace=1.2)

for i in range(solute_count):  
    # Plot bars for t=600
    print(sensitivity_change_plus20[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus20[i], height=bar_width, label=('Dwelltime 600'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for t=360 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus20[i], height=bar_width, label=('Dwelltime 360'),
        color=lighter_colors[i], alpha=0.5)
    
    # Annotate percentage change next to each bar
    for j, val in enumerate(sensitivity_change_plus20[i]):
        if val<=-1:
            axes[i].annotate(f'{val:.6f}%', xy=(val, j - bar_width/2), xytext=(-55, 0), textcoords='offset points',
                             fontsize=8, color='black', ha='left', va='center')
        else:
            axes[i].annotate(f'{val:.6f}%', xy=(val, j - bar_width/2), xytext=(5.5, 0), textcoords='offset points',
                             fontsize=8, color='black', ha='left', va='center')
        
    for j, val in enumerate(sensitivity_change_minus20[i]):
       if val<=-18:
           axes[i].annotate(f'{val:.6f}%', xy=(val, j + bar_width/2), xytext=(5, 0), textcoords='offset points',
                            fontsize=8, color='black', ha='left', va='center')
       elif val>10:
           axes[i].annotate(f'{val:.6f}%', xy=(val, j + bar_width/2), xytext=(-57, 0), textcoords='offset points',
                            fontsize=8, color='black', ha='left', va='center')
       elif 0<val<10:
           axes[i].annotate(f'{val:.6f}%', xy=(val, j + bar_width/2), xytext=(5, 0), textcoords='offset points',
                            fontsize=8, color='black', ha='left', va='center')
    ...
    axes[i].legend(fontsize='x-small',loc='lower right') 
    axes[i].set_yticks(np.arange(4)) 
    axes[i].set_yticklabels(modes)

plt.suptitle('Change in Solute Clearance Due to 25% Dwelltime Variation', fontsize=16)
