...
mean_values_TS_time_array=np.array(list(mean_values_TS_time.values()))    
reshaped_mean_values_TS_time=mean_values_TS_time_array.reshape(4,3,3)
mean_UR_clearances=reshaped_mean_values_TS_time[0,:,:]
mean_CR_clearances=reshaped_mean_values_TS_time[1,:,:]
mean_PhR_clearances=reshaped_mean_values_TS_time[2,:,:]
mean_PoR_clearances=reshaped_mean_values_TS_time[3,:,:]
...
std_values_TS_time_array=np.array(list(std_values_TS_time.values()))    
reshaped_std_values_TS_time=std_values_TS_time_array.reshape(4,3,3)
std_UR_clearances=reshaped_std_values_TS_time[0,:,:]
std_CR_clearances=reshaped_std_values_TS_time[1,:,:]
std_PhR_clearances=reshaped_std_values_TS_time[2,:,:]
std_PoR_clearances=reshaped_std_values_TS_time[3,:,:]
...
xtick_labels = [f" TS= {transport_status_key}, Time={t}" for transport_status_key in transport_status for t in t_values]
for axis in ax:
    axis.set_xticks(range(len(transport_status) * len(t_values)))
    axis.set_xticklabels(xtick_labels, rotation=35, fontsize=8)
    
fig.supxlabel('Transport Status (TS), Time (min)')
fig.supylabel('Total solute removal (mmol)')
fig.suptitle('Influence of Dwelltime on Solute Clearance in relation to Transport Status', fontsize=14)
...
file_path = 'trail2_patients_time.json'
...
df_from_json = pd.read_json('trail2_patients_time.json')

# Save DataFrame to a CSV file
df_from_json.to_csv('trial2_patients_time.csv', index=False)
