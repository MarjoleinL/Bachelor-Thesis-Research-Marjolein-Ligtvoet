    ...
    for t in t_values:    
        for j, mode in enumerate(modes):
            ...
            for DPI_key in ['low', 'mid', 'high']:
                ...
                '''Rates'''
                urea_grate = (DPI_value - 32.4)/7.23
                
                rate=np.array([urea_grate]) #Urea generation rate
...
fig, ax = plt.subplots(4, 1, figsize=(10,40), sharex=True ) 
...
fig.suptitle('Influence of Dwelltime on Solute Clearance in relation to Daily Protein Intake (DPI) in Males', fontsize=14)
...
xtick_labels = [f" Mean DPI= {mean_DPI[DPI_key]}, CD_Glu={gluc}" for DPI_key in DPI_values for t in t_values]
ax.set_xticks(range(len(DPI_values) * len(t_values)))
ax.set_xticklabels(xtick_labels, rotation=45)

fig.supxlabel('Mean DPI (g/day), time (min)')
fig.supylabel('Total solute removal (mmol)')
fig.suptitle('Influence of Dwelltime on Urea Clearance in relation to Daily Protein Intake (DPI) in Males', fontsize=14)
...
file_path = 'trail1_male_patients_time.json'
...
df_from_json = pd.read_json('trail1_male_patients_time.json')

# Save DataFrame to a CSV file
df_from_json.to_csv('trial1_male_patients_time.csv', index=False)
