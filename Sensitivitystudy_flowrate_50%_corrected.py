...
f_avg_values=([50,100,150])  #flowrate ml/min
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for flowrate 50
...
for i in range(solute_count):  
    # Plot bars for flowrate 150 
    print(sensitivity_change_plus50[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus50[i], height=bar_width, label=('Flowrate 150'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for flowrate 50 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus50[i], height=bar_width, label=('Flowrate 50'),
...
plt.suptitle('Change in Solute Clearance Due to 50% Flowrate Variation', fontsize=16)
