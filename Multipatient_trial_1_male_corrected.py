...
for patient_index in range(num_patients):
    ...
    for j, mode in enumerate(modes):
        ...
        for DPI_key in ['low', 'mid', 'high']:
            ...
            urea_plasma_concentration= ((((DPI_value+5.18-(1.89*BMI))/1.38)*SCR)*10)/molecular_weight['Urea'] #mmol/L 
...
#4. Calculate mean, min and max of [C00, c10, C20 and so on] -> low dpi This would be three arrays.
mean_urea_over_time_low = []
min_urea_over_time_low = []
max_urea_over_time_low= []

# Iterate over each time point
for t in range(480):
    # Collect the Urea values at time point t for all patients
    urea_values_at_t_low = [patient_data['low']['Urea'][t] for patient_index, patient_data in results_per_patient.items()]
    
    # Calculate the mean Urea value at time point t
    mean_urea_at_t_low = np.mean(urea_values_at_t_low)
    
    # Append the mean value to the list
    mean_urea_over_time_low.append(mean_urea_at_t_low)
    
    # Calculate the minimum Urea value at time point t
    min_urea_at_t_low = np.min(urea_values_at_t_low)
    
    # Append the minimum value to the list
    min_urea_over_time_low.append(min_urea_at_t_low)
    
    # Calculate the maximum Urea value at time point t
    max_urea_at_t_low = np.max(urea_values_at_t_low)
    
    # Append the minimum value to the list
    max_urea_over_time_low.append(max_urea_at_t_low)

#5. Calculate mean, min, max for mid and high
#mid
mean_urea_over_time_mid = []
min_urea_over_time_mid = []
max_urea_over_time_mid= []

# Iterate over each time point
for t in range(480):
    # Collect the Urea values at time point t for all patients
    urea_values_at_t_mid = [patient_data['mid']['Urea'][t] for patient_index, patient_data in results_per_patient.items()]
    
    # Calculate the mean Urea value at time point t
    mean_urea_at_t_mid = np.mean(urea_values_at_t_mid)
    
    # Append the mean value to the list
    mean_urea_over_time_mid.append(mean_urea_at_t_mid)
    
    # Calculate the minimum Urea value at time point t
    min_urea_at_t_mid = np.min(urea_values_at_t_mid)
    
    # Append the minimum value to the list
    min_urea_over_time_mid.append(min_urea_at_t_mid)
    
    # Calculate the maximum Urea value at time point t
    max_urea_at_t_mid = np.max(urea_values_at_t_mid)
    
    # Append the minimum value to the list
    max_urea_over_time_mid.append(max_urea_at_t_mid)

#high
mean_urea_over_time_high = []
min_urea_over_time_high = []
max_urea_over_time_high= []

# Iterate over each time point
for t in range(480):
    # Collect the Urea values at time point t for all patients
    urea_values_at_t_high = [patient_data['high']['Urea'][t] for patient_index, patient_data in results_per_patient.items()]
    
    # Calculate the mean Urea value at time point t
    mean_urea_at_t_high = np.mean(urea_values_at_t_high)
    
    # Append the mean value to the list
    mean_urea_over_time_high.append(mean_urea_at_t_high)
    
    # Calculate the minimum Urea value at time point t
    min_urea_at_t_high = np.min(urea_values_at_t_high)
    
    # Append the minimum value to the list
    min_urea_over_time_high.append(min_urea_at_t_high)
    
    # Calculate the maximum Urea value at time point t
    max_urea_at_t_high = np.max(urea_values_at_t_high)
    
    # Append the minimum value to the list
    max_urea_over_time_high.append(max_urea_at_t_high)
...
plt.plot(time, mean_urea_over_time_low, label='Low', color='green')
plt.fill_between(time, min_urea_over_time_low, max_urea_over_time_low, color='green', alpha=0.2)

plt.plot(time, mean_urea_over_time_mid, label='Mid', color='blue')
plt.fill_between(time, min_urea_over_time_mid, max_urea_over_time_mid, color='blue', alpha=0.1)

plt.plot(time, mean_urea_over_time_high, label='High', color='red')
plt.fill_between(time, min_urea_over_time_high, max_urea_over_time_high, color='red', alpha=0.2)

plt.xlabel('Time (min)')
plt.ylabel('Urea Dialysate Concentration (mmol/L)')
plt.title('Influence of Daily Protein Intake (DPI) on Urea Dialysate Concentrations in Males')
...
#Plots average clearances for each solute
fig, ax = plt.subplots(1, figsize=(8,6) )
solute_names = ['Urea']
mean_clearances = [mean_UR_clearances]
std_clearances = [std_UR_clearances]

for i, (solute, mean_values, std_values) in enumerate(zip(solute_names, mean_clearances, std_clearances)):
    ax.errorbar(range(len(DPI)), mean_values, yerr=std_values, fmt='o', capsize=5)
    ax.set_xticks(range(len(DPI)))
    tick_labels = [f"{DPI_key} = {mean_DPI[DPI_key]}" for DPI_key in ['low', 'mid', 'high']]
    ax.set_xticklabels(tick_labels, rotation=45)

fig.supxlabel('Daily Protein Intake (g/day)')
fig.supylabel('Total solute removal (mmol)')
fig.suptitle('Influence of DPI on Urea Clearance', fontsize=16)
...
import json
# Assuming your dictionary is named `results_per_patient`
...
file_path = 'trail1_male_patients.json'
...
df_from_json = pd.read_json('trail1_male_patients.json')

# Save DataFrame to a CSV file
df_from_json.to_csv('trial1_male_patients.csv', index=False)
