...
#volumes for -50%,  baseline and +50%
VDR_values = np.array([5000, 10000, 15000])
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for VDR 5000
...
for i in range(solute_count):  
    # Plot bars for VDR 15000
    print(sensitivity_change_plus50[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus50[i], height=bar_width, label=('VDR 15000'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for VDR 5000 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus50[i], height=bar_width, label=('VDR 5000'),
...
plt.suptitle('Change in Solute Clearance Due to 50% VDR Variation', fontsize=16)
