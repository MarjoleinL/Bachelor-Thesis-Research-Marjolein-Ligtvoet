...
patient_data = {}  # Dictionary (smaller, patient specific) to store data for each patien
...
V[480] = 149 #ml
V_fill =  233 #ml
Vr_values = np.array([32, 40, 48]) #ml values for -20%, baseline and +20%
for Vr in Vr_values:
    V[0] = V_fill + Vr   
...
# Calculate percentage change compared to Vr 40
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for Vr 32
...
for i in range(solute_count):  
    ...
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus20[i], height=bar_width, label=('Vr 48'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for Vr 32 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus20[i], height=bar_width, label=('Vr 32'),
...
plt.suptitle('Change in Solute Clearance Due to 20% Residual Volume Variation', fontsize=16)
