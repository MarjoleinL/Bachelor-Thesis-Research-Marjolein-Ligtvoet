...
#values for -20%, baseline and +20%
x_AC_values = np.array([240, 300, 360])
...
# Calculate percentage change compared to sorbent weight 300
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for sorbent weight 240
...
for i in range(solute_count):  
    # Plot bars for sorbent weight 360
    print(sensitivity_change_plus20[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus20[i], height=bar_width, label=('Sorbent Weight 360'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for sorbent weight 240 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus20[i], height=bar_width, label=('Sorbent Weight 240'),
    ...
    for j, val in enumerate(sensitivity_change_plus20[i]):
        if val<-0.0005:
            ...
            axes[i].annotate(f'{val:.6f}%', xy=(val, j - bar_width/2), xytext=(-50, 0), textcoords='offset points',
                             fontsize=8, color='black', ha='left', va='center')
...
plt.suptitle('Change in Solute Clearance Due to 20% Sorbent Weight Variation', fontsize=16)
