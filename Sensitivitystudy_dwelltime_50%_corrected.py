...
#t = 480 #min
#values for -50%, baseline and +50%
t_values = np.array([240, 480, 720]) 
...
additional_time_points = np.arange(481, 721)
...
solute_colors = ['DarkGreen','Navy','DarkOrange','DarkViolet']  # Define colors for each solute for sensitivity_change_plus50
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for sensitivity_change_minus50
...
for i in range(solute_count):  
    ...
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus50[i], height=bar_width, label=('Dwelltime 720'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for t=240 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus50[i], height=bar_width, label=('Dwelltime 240'),
...
plt.suptitle('Change in Solute Clearance Due to 50% Dwelltime Variation', fontsize=16)
