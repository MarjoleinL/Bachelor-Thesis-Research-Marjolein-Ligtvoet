...
f_avg_values=([80,100,120]) #flowrate ml/min


for f_avg in f_avg_values:
...
# Calculate percentage change compared to flowrate 100
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for flowrate 80
...
for i in range(solute_count):  
    # Plot bars for flowrate 120
    print(sensitivity_change_plus20[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus20[i], height=bar_width, label=('Flowrate 120'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for flowrate 80 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus20[i], height=bar_width, label=('Flowrate 80'),
...
plt.suptitle('Change in Solute Clearance Due to 20% Flowrate Variation', fontsize=16)
