...
for i, (solute, mean_values, std_values) in enumerate(zip(solute_names, mean_clearances, std_clearances)):
    ax[i].errorbar(range(len(transport_status)), mean_values, yerr=std_values, fmt='o', capsize=5)
    ax[i].set_xticks(range(len(transport_status)))
    tick_labels = [f"{transport_status_key}" for transport_status_key in ['slow', 'average', 'fast']]
...
fig.supxlabel('Transport Status')
fig.supylabel('Total solute removal (mmol)')
fig.suptitle('Influence of Transport Status on Clearance', fontsize=16)
...
#1. Calculate for patient 0 -> c_urea low mid high C00, c01, c02
#2. for patient 1 -> C10, c11, c12
#3. repeat for other patients.
## dilaysate urea concentration for each patient is in results per patient--> patient data --> high, low, mid

#4. Calculate mean, min and max of [C00, c10, C20 and so on] -> slow dpi This would be three arrays.
mean_over_time_slow = {solute: [] for solute in solute_names}
min_over_time_slow = {solute: [] for solute in solute_names}
max_over_time_slow = {solute: [] for solute in solute_names}

# Iterate over each time point
for t in range(480):
    for solute in solute_names:
        # Collect the Urea values at time point t for all patients
        values_at_t_slow = [patient_data['slow'][solute][t] for patient_index, patient_data in results_per_patient.items()]
        
        # Calculate the mean Urea value at time point t
        mean_at_t_slow = np.mean(values_at_t_slow)
        
        # Calculate the minimum Urea value at time point t
        min_at_t_slow = np.min(values_at_t_slow)
        
        # Calculate the maximum Urea value at time point t
        max_at_t_slow = np.max(values_at_t_slow)
    
        # Append the mean value to the list
        mean_over_time_slow[solute].append(mean_at_t_slow)
        # Append the minimum value to the list
        min_over_time_slow[solute].append(min_at_t_slow)
        # Append the minimum value to the list
        max_over_time_slow[solute].append(max_at_t_slow)
    
        
#5. Calculate mean, min, max for average and fast
#average
mean_over_time_average = {solute: [] for solute in solute_names}
min_over_time_average = {solute: [] for solute in solute_names}
max_over_time_average = {solute: [] for solute in solute_names}

# Iterate over each time point
for t in range(480):
    for solute in solute_names:
        # Collect the Urea values at time point t for all patients
        values_at_t_average = [patient_data['average'][solute][t] for patient_index, patient_data in results_per_patient.items()]
        
        # Calculate the mean Urea value at time point t
        mean_at_t_average = np.mean(values_at_t_average)
        
        # Calculate the minimum Urea value at time point t
        min_at_t_average = np.min(values_at_t_average)
        
        # Calculate the maximum Urea value at time point t
        max_at_t_average = np.max(values_at_t_average)
        
        # Append the mean value to the list
        mean_over_time_average[solute].append(mean_at_t_average)
        # Append the minimum value to the list
        min_over_time_average[solute].append(min_at_t_average)
        # Append the minimum value to the list
        max_over_time_average[solute].append(max_at_t_average)
    
#fast
mean_over_time_fast = {solute: [] for solute in solute_names}
min_over_time_fast = {solute: [] for solute in solute_names}
max_over_time_fast = {solute: [] for solute in solute_names}

# Iterate over each time point
for t in range(480):
    for solute in solute_names:
        # Collect the Urea values at time point t for all patients
        values_at_t_fast = [patient_data['fast'][solute][t] for patient_index, patient_data in results_per_patient.items()]
        
        # Calculate the mean Urea value at time point t
        mean_at_t_fast = np.mean(values_at_t_fast)
         
        # Calculate the minimum Urea value at time point t
        min_at_t_fast = np.min(values_at_t_fast)
        
        # Calculate the maximum Urea value at time point t
        max_at_t_fast = np.max(values_at_t_fast)

        # Append the mean value to the list
        mean_over_time_fast[solute].append(mean_at_t_fast)
        # Append the minimum value to the list
        min_over_time_fast[solute].append(min_at_t_fast)
        # Append the minimum value to the list
        max_over_time_fast[solute].append(max_at_t_fast)
        
    

time = np.arange(0, 480)

# Plotting the data Creatinine
plt.figure(figsize=(10, 6))
plt.plot(time, mean_over_time_slow['Creatinine'], label='Slow', color='green')
plt.fill_between(time, min_over_time_slow['Creatinine'], max_over_time_slow['Creatinine'], color='green', alpha=0.2)

plt.plot(time, mean_over_time_average['Creatinine'], label='Average', color='blue')
plt.fill_between(time, min_over_time_average['Creatinine'], max_over_time_average['Creatinine'], color='blue', alpha=0.1)

plt.plot(time, mean_over_time_fast['Creatinine'], label='Fast', color='red')
plt.fill_between(time, min_over_time_fast['Creatinine'], max_over_time_fast['Creatinine'], color='red', alpha=0.2)

plt.xlabel('Time (min)')
plt.ylabel('Dialysate Concentration (mmol/L)')
plt.title('Influence of Transport Status on Creatinine Dialysate Concentrations')
plt.legend(loc='upper left')
plt.grid(True)
plt.show()

# Plotting the data Phosphate
plt.figure(figsize=(10, 6))
plt.plot(time, mean_over_time_slow['Phosphate'], label='Slow', color='green')
plt.fill_between(time, min_over_time_slow['Phosphate'], max_over_time_slow['Phosphate'], color='green', alpha=0.2)

plt.plot(time, mean_over_time_average['Phosphate'], label='Average', color='blue')
plt.fill_between(time, min_over_time_average['Phosphate'], max_over_time_average['Phosphate'], color='blue', alpha=0.1)

plt.plot(time, mean_over_time_fast['Phosphate'], label='Fast', color='red')
plt.fill_between(time, min_over_time_fast['Phosphate'], max_over_time_fast['Phosphate'], color='red', alpha=0.2)

plt.xlabel('Time (min)')
plt.ylabel('Dialysate Concentration (mmol/L)')
plt.title('Influence of Transport Status on Phosphate Dialysate Concentrations')
plt.legend(loc='upper left')
plt.grid(True)
plt.show()

# Plotting the data Potassium
plt.figure(figsize=(10, 6))
plt.plot(time, mean_over_time_slow['Potassium'], label='Slow', color='green')
plt.fill_between(time, min_over_time_slow['Potassium'], max_over_time_slow['Potassium'], color='green', alpha=0.2)

plt.plot(time, mean_over_time_average['Potassium'], label='Average', color='blue')
plt.fill_between(time, min_over_time_average['Potassium'], max_over_time_average['Potassium'], color='blue', alpha=0.1)

plt.plot(time, mean_over_time_fast['Potassium'], label='Fast', color='red')
plt.fill_between(time, min_over_time_fast['Potassium'], max_over_time_fast['Potassium'], color='red', alpha=0.2)

plt.xlabel('Time (min)')
plt.ylabel('Dialysate Concentration (mmol/L)')
plt.title('Influence of Transport Status on Potassium Dialysate Concentrations')
plt.legend(loc='upper left')
plt.grid(True)
plt.show()

# Plotting the data Urea
plt.figure(figsize=(10, 6))
plt.plot(time, mean_over_time_slow['Urea'], label='Slow', color='green')
plt.fill_between(time, min_over_time_slow['Urea'], max_over_time_slow['Urea'], color='green', alpha=0.2)

plt.plot(time, mean_over_time_average['Urea'], label='Average', color='blue')
plt.fill_between(time, min_over_time_average['Urea'], max_over_time_average['Urea'], color='blue', alpha=0.1)

plt.plot(time, mean_over_time_fast['Urea'], label='Fast', color='red')
plt.fill_between(time, min_over_time_fast['Urea'], max_over_time_fast['Urea'], color='red', alpha=0.2)

plt.xlabel('Time (min)')
plt.ylabel('Dialysate Concentration (mmol/L)')
plt.title('Influence of Transport Status on Urea Dialysate Concentrations')
plt.legend(loc='upper left')
plt.grid(True)
plt.show()
...
file_path = 'trail2_patients.json'
...
df_from_json = pd.read_json('trail2_patients.json')

# Save DataFrame to a CSV file
df_from_json.to_csv('trial2_patients.csv', index=False)

for patient_index, patient_data in results_per_patient.items():
    print(patient_data['Plasma_Solute_Concentration']['Urea'][0])
    print(patient_data['Plasma_Solute_Concentration']['Creatinine'][0])
    print(patient_data['Plasma_Solute_Concentration']['Sodium'][0])
    print(patient_data['Plasma_Solute_Concentration']['Phosphate'][0])
    print(patient_data['Plasma_Solute_Concentration']['Glucose'][0])
    print(patient_data['Plasma_Solute_Concentration']['Potassium'][0])
for patient_index, patient_data in results_per_patient.items():    
    print(patient_data['MTAC'][0])
    print(patient_data['MTAC'][1])
    print(patient_data['MTAC'][2])
    print(patient_data['MTAC'][3])
    print(patient_data['MTAC'][4])
    print(patient_data['MTAC'][5])
