...
x_AC_values = np.array([150, 300, 450])
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for sorbent weight 150
...
for i in range(solute_count):  
    # Plot bars for sorbent weight 450
    print(sensitivity_change_plus50[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus50[i], height=bar_width, label=('Sorbent Weight 450'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for sorbent weight 150 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus50[i], height=bar_width, label=('Sorbent Weight 150'),
...
plt.suptitle('Change in Solute Clearance Due to 50% Sorbent Weight Variation', fontsize=16)
