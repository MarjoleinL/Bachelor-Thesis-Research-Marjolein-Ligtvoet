...
V = np.zeros(481)
V[480] = 149 #ml
V_fill_values = np.array([186, 233, 280]) #ml values for roughly  -20%,baseline, roughly +20%
Vr = 40 #ml
...
lighter_colors = ['SpringGreen','LightSkyBlue','Orange','Violet' ] # Define a lighter color for v_fill 186
...
for i in range(solute_count):  
    # Plot bars for  V_fill 280
    print(sensitivity_change_plus20[i])
    axes[i].barh(np.arange(4)-bar_width/2, sensitivity_change_plus20[i], height=bar_width, label=('V_fill 280'),
        color=solute_colors[i], alpha=0.7)
    # Plot bars for v_fill 186 with a lighter shade
    axes[i].barh(np.arange(4) + bar_width/2, sensitivity_change_minus20[i], height=bar_width, label=('V_fill 186'),
...
plt.suptitle('Change in Solute Clearance Due to 20% Fill Volume Variation', fontsize=16)
